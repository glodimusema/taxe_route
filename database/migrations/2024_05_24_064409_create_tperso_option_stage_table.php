...
class CreateTpersoOptionStageTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('tperso_option_stage', function (Blueprint $table) {
            $table->id();
            $table->string('name_option');
            $table->foreignId('domaine_id')->constrained('tperso_domaine_stage')->cascadeOnUpdate()->cascadeOnDelete(); 
            $table->string('author'); 
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('tperso_option_stage');
    }
}
