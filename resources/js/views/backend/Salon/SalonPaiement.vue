<template>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_vente_paiement_salon/${this.svData.id}`,
            ...
            this.svData.libellepaie = 'Paiement Facture';
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_vente_paiement_salon`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_paiement_salon/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_vente_paiement_salon/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_vente_paiement_salon/${this.refEnteteVente}?page=`);
      },
      backPage() {
        this.$router.go(-1);
      },
...
//
      var tables='tsalon_paiement';
      var user_name=this.userData.name;
      var user_id=this.userData.id;
      var detail_information="Suppression d'un paiement du montant de : "+noms+" en date du "+datepaie+" pour la vente des services du salon de coiffure n° "+numEntete+" par l'utilisateur "+user_name+"" ;
