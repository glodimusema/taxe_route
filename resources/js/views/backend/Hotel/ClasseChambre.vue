                    ...
                    <v-card :loading="loading">
                        <v-form ref="form" lazy-validation>
                            ...
                            <v-card-text>
                                ...
                                <v-text-field label="Prix" prepend-inner-icon="extension"
                                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                                    v-model="svData.prix_chambre"></v-text-field>
...
export default {
    ...
    data() {
        return {
            ...
            //'id','designation','prix_chambre','devise','taux','author'
    ...
    methods: {
        ...
        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout Classe Chambre ";
            this.edit = false;
        ...
        //   }, 300),
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_hotel_classe_chambre?page=`);
        },
        ...
        validate() {
            if (this.$refs.form.validate()) {
                ...
                this.insertOrUpdate(
                    `${this.apiBaseURL}/insert_hotel_classe_chambre`,
        ...
        editData(id) {
            this.editOrFetch(`${this.apiBaseURL}/fetch_single_hotel_classe_chambre/${id}`).then(
                ({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_hotel_classe_chambre/${id}`).then(
                    ({ data }) => {
