...
export default {
  data() {
    return {
      ...
      totalPaie: 0,
      RestePaie: 0,
      // refReservation'id','refReservation','montant_paie','date_paie','modepaie','libellepaie','author'
      svData: {
        id: '',
        refReservation: 0,
        montant_paie: 0,
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        this.isLoading(true);
        if (this.edit) {
          this.svData.refReservation = this.refReservation
          this.svData.author = this.userData.name;
          this.svData.libellepaie = 'Paiement Facture';
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_hotel_paiement_salle/${this.svData.id}`,
          ...
          this.svData.libellepaie = 'Paiement Facture';
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_hotel_paiement_salle`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    // s'id','refReservation','refFrais','puEntree','qteEntree','author'
    //   this.fetchDataList();
    // }, 300),

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_hotel_paiement_salle/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
            this.svData.refReservation = item.refReservation;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_hotel_paiement_salle/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_hotel_paiement_salle/${this.refReservation}?page=`);
    },
    backPage() {
      this.$router.go(-1);
    },
    desactiverData(valeurs, user_created, date_entree, noms, numEntete, datepaie) {
      //
      var tables = 'thotel_paiement_salle';
      var user_name = this.userData.name;
      var user_id = this.userData.id;
      var detail_information = "Suppression d'un paiement du montant de : " + noms + " en date du " + datepaie + " pour la Reservation des Salles n° " + numEntete + " par l'utilisateur " + user_name + "";
