<template>
  <v-row justify="center">
    <v-dialog v-model="etatModal" persistent max-width="1300px">
      <v-card>
        <!-- container -->
...
export default {
  data() {
    return {
      ...
      totalPaie: 0,
      RestePaie: 0,
      // 'id','refReservation','montant_paie','date_paie','modepaie','libellepaie','author'
      svData: {
        id: '',
        refReservation: 0,
        montant_paie: 0,
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_hotel_paiement_chambre/${this.svData.id}`,
          ...
          this.svData.libellepaie = 'Paiement Facture';
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_hotel_paiement_chambre`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_hotel_paiement_chambre/${id}`).then(
        ({ data }) => {
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_hotel_paiement_chambre/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_hotel_paiement_chambre/${this.refReservation}?page=`);
    },
    backPage() {
      this.$router.go(-1);
    },
