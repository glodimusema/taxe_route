                ...
                <v-dialog v-model="dialog" max-width="500px" persistent>
                  <v-card :loading="loading">
                    <v-form ref="form" lazy-validation>
                      <v-card-title>
                        Recette sur Vente des Produits <v-spacer></v-spacer>
                      ...
                      <v-card-text>
                        ...
                        <v-layout row wrap>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-text-field type="number" label="Montant Déposé" prepend-inner-icon="extension" dense
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Device" :items="[
                                ...
                                item-text="designation" item-value="designation" v-model="svData.devise"></v-autocomplete>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                            <div class="mr-1">
                              <v-text-field label="Info. Sur la devise du montant deposé(FC ou USD)" prepend-inner-icon="extension" dense
                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                                v-model="svData.Info_devise"></v-text-field>
                            </div>
                          </v-flex>
                          ...
                          <template v-slot:default>
                            <thead>
                              <tr>  
                                ...
                                <th class="text-left">InfoDevise</th>
                            ...
                            <tbody>
                              <tr v-for="item in fetchData" :key="item.id">
                                ...
                                <td>{{ item.montant_paie }}FC</td>
                                <td>{{ item.modepaie }}</td>
                                <td>{{ item.date_paie }}</td>
                                <td>{{ item.taux }}</td>
                                <td>{{ item.Info_devise }}</td>
                                ...
                                <td>  
                                  ...
                                  <v-menu bottom rounded offset-y transition="scale-transition">
                                    ...
                                    <v-list dense width="">
                                      ...
                                      <v-list-item link @click="desactiverData(item.id, item.author, item.created_at, item.montant_paie, item.refEnteteMvt,item.date_paie)">
                                        <v-list-item-icon>
  ...
  export default {
    data() {
      return {
        ...
        // 'id','refEnteteMvt','montant_paie','devise','taux','date_paie','modepaie','libellepaie','refBanque','numeroBordereau','author'
        svData: {
          id: '',
          refEnteteMvt: 0,
          montant_paie: 0,
    ...
    methods: {
      ...
      async get_Banque() {
        this.isLoading(true);
        await axios
          .get(`${this.apiBaseURL}/fetch_list_banque/${this.modepaie}`)
          .then((res) => {
            var chart = res.data.data;
            if (chart) {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.libellepaie = 'Depot pour vente des produit';
            this.svData.modepaie=this.modepaie;
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_car_paiement/${this.svData.id}`,
            ...
            this.svData.modepaie=this.modepaie;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_car_paiement`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      // s'id','refEnteteMvt','refFrais','puEntree','qteEntree','author'
      //   this.fetchDataList();
      // }, 300),
  
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_car_paiement/${id}`).then(
          ({ data }) => {
            ...
            donnees.map((item) => {
              ...
              this.svData.date_paie = item.date_paie;
              ...
              this.svData.Info_devise = item.Info_devise;
              //Info_devise
            });
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_car_paiement/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_car_paiement_Movement/${this.refEnteteMvt}?page=`);
      },
      backPage() {
        this.$router.go(-1);
      },
...
//
      var tables='tcar_paiement';
      var user_name=this.userData.name;
      var user_id=this.userData.id;
      var detail_information="Suppression le depot du montant de : "+noms+" en date du "+datepaie+" pour le mouvement n° "+numEntete+" par l'utilisateur "+user_name+"" ;
