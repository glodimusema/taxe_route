                                  ...
                                  <td>
                                    ...
                                    <v-menu bottom rounded offset-y transition="scale-transition">
                                      ...
                                      <v-list dense width="">
                                        ...
                                        <v-list-item link @click="printBill(item.pdfMouvement)">
                                        <v-list-item-icon>
    ...
    export default {
      ...
      data() {
        return {
          ...
          // 'id','refEnteteMvt','pdfMouvement','pdfMouvement','author'
          svData: {
            id: '',
            refEnteteMvt: 0,        
            desicriptionPDF: "",       
      ...
      methods: {
        ...
        updatePhoto() {
          ...
          if (this.edit == true) {
            ...
            axios
              .post(`${this.apiBaseURL}/update_annexe_mouvement`, formData, config)
              .then(({ data }) => {
                this.image = "";
            ...
            this.svData.author = this.userData.name;
            axios
              .post(`${this.apiBaseURL}/insert_annexe_mouvement`, formData, config)
              .then(({ data }) => {
                this.image = "";
                this.showMsg(data.data);
        ...
        printBill(filenamess) {
          window.open(`${this.apiBaseURL}/downloadfile_mouvement/${filenamess}`);
        },
    
        editData(id) {
          this.editOrFetch(`${this.apiBaseURL}/fetch_single_annexe_mouvement/${id}`).then(
            ({ data }) => {
        ...
        deleteData(id) {
          this.confirmMsg().then(({ res }) => {
            this.delGlobal(`${this.apiBaseURL}/delete_annexe_mouvement/${id}`).then(
              ({ data }) => {
                this.showMsg(data.data);
                this.fetchDataList();
              }
        ...
        fetchDataList() {
               this.fetch_data(`${this.apiBaseURL}/fetch_annexe_mouvement/${this.refEnteteMvt}?page=`);
          //
        },
    desactiverData(valeurs,user_created,date_entree,noms,numEntete) {
//
      var tables='tcar_annexe';
      var user_name=this.userData.name;
      var user_id=this.userData.id;
      var detail_information="Suppression de l'annexe : "+noms+" sur la facture n° "+numEntete+" par l'utilisateur "+user_name+"" ;
