<template>
                      ...
                      <v-form ref="form" lazy-validation>
                        ...
                        <v-card-text>
                          ...
                          <v-text-field type="number" label="Quantité " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.qteEmballage">
                          </v-text-field>
    
                          <v-text-field type="number" label="Prix Unitaire ($) " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.puEmballage">
                          </v-text-field>
                            ...
                            <template v-slot:default>
                              ...
                              <tbody>
                                <tr v-for="item in fetchData" :key="item.id">
                                  ...
                                  <td>{{ item.qteEmballage }}</td>
                                  <td>{{ item.puEmballage }}</td>
                                  <td>{{ item.PTEmballage }}</td>
    ...
    export default {
      ...
      methods: {
        ...
        validate() {
          if (this.$refs.form.validate()) {
            ...
            if (this.edit) {
              ...
              this.insertOrUpdate(
                `${this.apiBaseURL}/update_car_emballage/${this.svData.id}`,
              ...
              this.svData.author = this.userData.name;
              this.insertOrUpdate(
                `${this.apiBaseURL}/insert_car_emballage`,
                JSON.stringify(this.svData)
              )
                .then(({ data }) => {
                  this.showMsg(data.data);
        ...
        // s'id','refEnteteMvt','refProduit','puEmballage','qteEmballage','author'
        //   this.fetchDataList();
        // }, 300),
    
        editData(id) {
          this.editOrFetch(`${this.apiBaseURL}/fetch_single_car_emballage/${id}`).then(
            ({ data }) => {
              ...
              donnees.map((item) => {
                ...
                this.svData.puEmballage = item.puEmballage;
                this.svData.qteEmballage = item.qteEmballage;
        ...
        deleteData(id) {
          this.confirmMsg().then(({ res }) => {
            this.delGlobal(`${this.apiBaseURL}/delete_car_emballage/${id}`).then(
              ({ data }) => {
                this.showMsg(data.data);
                this.fetchDataList();
              }
        ...
        fetchDataList() {
          this.fetch_data(`${this.apiBaseURL}/fetch_car_emballage_Movement/${this.refEnteteMvt}?page=`);
        },
      ...
      getPrice(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_car_produit/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {
              this.svData.puEmballage = item.pu;      
            });
...
//
      var tables='tcar_emballage';
      var user_name=this.userData.name;
      var user_id=this.userData.id;
      var detail_information="Suppression du produit : "+noms+" sur les Emballages du mouvement n° "+numEntete+" par l'utilisateur "+user_name+"" ;
