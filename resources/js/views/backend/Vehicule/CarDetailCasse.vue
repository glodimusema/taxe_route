<template>
                      ...
                      <v-form ref="form" lazy-validation>
                        ...
                        <v-card-text>
                          ...
                          <v-text-field type="number" label="Quantité " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.qteCasse">
                          </v-text-field>
    
                          <v-text-field type="number" label="Prix Unitaire ($) " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.puCasse">
                          </v-text-field>
    ...
    export default {
      ...
      methods: {
        ...
        validate() {
          if (this.$refs.form.validate()) {
            ...
            if (this.edit) {
              ...
              this.insertOrUpdate(
                `${this.apiBaseURL}/update_car_detail_casse/${this.svData.id}`,
              ...
              this.svData.author = this.userData.name;
              this.insertOrUpdate(
                `${this.apiBaseURL}/insert_car_detail_casse`,
                JSON.stringify(this.svData)
              )
                .then(({ data }) => {
                  this.showMsg(data.data);
        ...
        // s'id','refEnteteMvt','refProduit','puCasse','qteCasse','author'
        //   this.fetchDataList();
        // }, 300),
    
        editData(id) {
          this.editOrFetch(`${this.apiBaseURL}/fetch_single_car_detail_casse/${id}`).then(
            ({ data }) => {
              ...
              donnees.map((item) => {
                ...
                this.svData.puCasse = item.puCasse;
                this.svData.qteCasse = item.qteCasse;
        ...
        deleteData(id) {
          this.confirmMsg().then(({ res }) => {
            this.delGlobal(`${this.apiBaseURL}/delete_car_detail_casse/${id}`).then(
              ({ data }) => {
                this.showMsg(data.data);
                this.fetchDataList();
              }
        ...
        fetchDataList() {
          this.fetch_data(`${this.apiBaseURL}/fetch_car_detail_casse_Movement/${this.refEnteteMvt}?page=`);
        },
...
//
      var tables='tcar_detail_casse';
      var user_name=this.userData.name;
      var user_id=this.userData.id;
      var detail_information="Suppression du produit : "+noms+" dans les Casses du mouvement n° "+numEntete+" par l'utilisateur "+user_name+"" ;
