<template>
          ...
          <v-dialog v-model="dialog" max-width="400px" scrollable  transition="dialog-bottom-transition">
            <v-card :loading="loading">
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    label="nom_emplacement"
                    prepend-inner-icon="extension"
                    :rules="[(v) => !!v || 'Ce champ est requis']"
                    outlined
                    v-model="svData.nom_emplacement"
                  ></v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Emplacement ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_emplacement;
        } else {
          this.titleComponent = "Ajout pays ";
        }
      },
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_log_emplacement?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_log_emplacement`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_log_emplacement/${id}`).then(
          ({ data }) => {
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_log_emplacement/${id}`).then(
            ({ data }) => {
