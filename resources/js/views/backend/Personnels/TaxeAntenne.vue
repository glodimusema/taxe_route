            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_antene">
                </v-text-field>
              </v-card-text>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Antenne";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_antene;
      } else {
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_taxe_antene?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_taxe_antene`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_taxe_antene/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_taxe_antene/${id}`).then(
          ({ data }) => {
