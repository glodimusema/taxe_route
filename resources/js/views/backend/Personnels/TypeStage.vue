              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_typestage"></v-text-field> 
  ...
  export default {
    ...
    data() {
      return {
        ...
        disabled: false,
        edit: false,
        //tperso_annee_stage id,name_typestage,author
        svData: {
          id: "",
          name_typestage: "",
          author:""
    ...
    methods: {
      ...
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_type_stage?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_type_stage`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_type_stage/${id}`).then(
          ({ data }) => {
            var donnees = data.data;  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.name_typestage;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_type_stage/${id}`).then(
            ({ data }) => {
