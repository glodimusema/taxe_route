<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_domaine"></v-text-field> 
  ...
  export default {
    ...
    methods: {
      ...
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_domaine_stage?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_domaine_stage`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_domaine_stage/${id}`).then(
          ({ data }) => {
            var donnees = data.data;  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.name_domaine;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_domaine_stage/${id}`).then(
            ({ data }) => {
