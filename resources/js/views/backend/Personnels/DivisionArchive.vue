<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_division"></v-text-field>
  
                    <v-text-field label="Description" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.description_division"></v-text-field>
                </v-card-text>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      ...
                      <td>{{ item.description_division }}</td>
                      <td>{{ item.author }}</td>
                      <td>
  ...
  export default {
    ...
    methods: {
      ...
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_perso_division?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_perso_division`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_perso_division/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.name_division;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_perso_division/${id}`).then(
            ({ data }) => {
