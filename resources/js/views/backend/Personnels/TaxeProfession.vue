<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_profession">
                  </v-text-field>
                  ...
                  <v-autocomplete label="Selectionnez le Secteur" prepend-inner-icon="mdi-map"
                     ...
                     item-text="nom_secteur" item-value="id" outlined v-model="svData.id_Secteur">
                </v-autocomplete>
                </v-card-text>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Profession";
        this.edit = false;
      ...
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_taxe_profession?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_taxe_profession`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_taxe_profession/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_profession;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_profession/${id}`).then(
            ({ data }) => {
        ...
        fetchListSecteur() {
            this.editOrFetch(`${this.apiBaseURL}/fetch_taxe_secteur2`).then(
                ({ data }) => {
                    var donnees = data.data;
