<template>
                      ...
                      <v-form ref="form" lazy-validation>
                        ...
                        <v-card-text>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field type="number" label="Montant decaissé(USD)" prepend-inner-icon="event" dense
                                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.montant_projet">
                                </v-text-field>
                              </div>
                            </v-flex> 
                        ...
                        <v-card-text>
                          <v-simple-table>
                            <template v-slot:default>
                              <thead>
                                <!-- //  id','activite_id', 'affectation_id', 'montant_projet', 'author' -->
                                <tr>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_perso_paie_projet/${this.svData.id}`,
            ...
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_perso_paie_projet`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_perso_paie_projet/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_perso_paie_projet/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_perso_paie_projet/${this.activite_id}?page=`);
      },
