            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_poste">
                </v-text-field>

                <v-text-field label="Description" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.description_poste">
                </v-text-field>

                <v-text-field type="number" label="Montant de Transport" prepend-inner-icon="extension" dense
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Poste";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_poste;
      } else {
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_poste_pers?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_poste_pers`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_poste_pers/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_poste_pers/${id}`).then(
          ({ data }) => {
