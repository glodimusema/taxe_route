            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_mutuelle"></v-text-field>

                  <v-text-field label="Description" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.description_mutuelle"></v-text-field>
              </v-card-text>
              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in fetchData" :key="item.id">
                    ...
                    <td>{{ item.description_mutuelle }}</td>
                    <td>
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_mutuelle;
      } else {
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_mutuelle_pers?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_mutuelle_pers`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_mutuelle_pers/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_mutuelle_pers/${id}`).then(
          ({ data }) => {
