                    ...
                    <v-form ref="form" lazy-validation>
                        ...
                        <v-card-text max-height="1500px" background-color: white>
                            <v-layout row wrap>
                                ...
                                <v-flex xs12 sm12 md6 lg6>
                                    <div class="mr-1">
                                        <v-autocomplete label="Selectionnez l'adresse Mail" prepend-inner-icon="mdi-map"
                                            :rules="[(v) => !!v || 'Ce champ est requis']" :items="user_mailList"
                                            item-text="email" item-value="email" dense outlined v-model="svData.mail_profil"
                                            chips clearable>
                                        </v-autocomplete>
                                    </div>
                                </v-flex>
...
import RDVPatient from './RDVPatient.vue';
...
export default {
    ...
    methods: {
        ...
        fetchListSelection() {
            this.editOrFetch(`${this.apiBaseURL}/fetch_user_compte`).then(
                ({ data }) => {
                    var donnees = data.data;
                    this.user_mailList = donnees;
        ...
        onPageChange() {
            ...
            this.fetch_data(`${this.apiBaseURL}/fetch_carte_malade?page=`);
        },
        ...
        updatePhoto() {
            ...
            if (this.edit == true) {
                axios
                    .post(`${this.apiBaseURL}/update_carte_malade`, formData, config)
                    .then(({ data }) => {
                        this.image = "";
        ...
        showRapportMedical(refPatient, name) {
            ...
            if (refPatient != '') {
                ...
                this.$refs.RapportMedical.$data.refPatient = refPatient;
                this.$refs.RapportMedical.$data.svData.refPatient = refPatient;
        ...
        showRDVPatient(numeroCarte, name) {
            ...
            if (numeroCarte != '') {
                ...
                this.$refs.RDVPatient.$data.etatModal = true;
                this.$refs.RDVPatient.$data.numeroCarte = numeroCarte;
                this.$refs.RDVPatient.$data.svData.numeroCarte = numeroCarte;
                this.$refs.RDVPatient.fetchDataList();

                this.$refs.RDVPatient.$data.titleComponent =
