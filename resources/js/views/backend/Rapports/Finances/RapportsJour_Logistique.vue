                        ...
                        <v-row v-show="showDate">
                            ...
                            <v-col cols="12" sm="6">
                                    ...
                                    <template v-slot:activator="{ on, attrs }">
                                        <span v-bind="attrs" v-on="on">
                                            <v-btn @click="showFicheStockByDate" block color="  blue" dark>
                                                <v-icon>print</v-icon> FICHE DE STOCK
                                            </v-btn>
                                        </span>
                                    </template>
                                    <span>Imprimer le rapport</span>
                                ...
                                <v-flex xs12 sm12 md12 lg12>
                                    <div class="mr-1">
                                        <v-autocomplete label="Selectionnez le Service Personnel" prepend-inner-icon="map"
                                            ...
                                            v-model="svData.refService" clearable chips>
                                        </v-autocomplete>
                                    </div>
                                </v-flex>
                                <br>
                                ...
                                <v-tooltip bottom color="black">
                                    <template v-slot:activator="{ on, attrs }">
                                        <span v-bind="attrs" v-on="on">
                                            <v-btn @click="showDetailSortieByDate_Service" block color="  blue" dark>
                                ...
                                <v-flex xs12 sm12 md12 lg12>
                                    <div class="mr-1">
                                        <v-autocomplete label="Selectionnez l'emplacement" prepend-inner-icon="mdi-map"
                                            dense :rules="[(v) => !!v || 'Ce champ est requis']" :items="emplacementList"
                                            item-text="nom_emplacement" item-value="id" outlined
                                            v-model="svData.idEmplacement">
                                        </v-autocomplete>
                                    </div>
                                </v-flex>
                                <!-- <br> -->
                                ...
                                <v-tooltip bottom color="black">
                                    <template v-slot:activator="{ on, attrs }">
                                        <span v-bind="attrs" v-on="on">
                                            <v-btn @click="showFicheStockByDate_Empplacement" block color="  blue" dark>
                                                <v-icon>print</v-icon> FICHE DE STOCK/EMPLACEMENT
                                            </v-btn>
                                        </span>
                                    </template>
                                    <span>Imprimer le rapport</span>
...
export default {
    ...
    methods: {
        ...
        showFicheStockByDate_Categorie() {
            ...
            if (date1 <= date2) {
                ...
                if (this.svData.idCategorie != "") {
                    window.open(`${this.apiBaseURL}/pdf_fiche_stock_logistique_cayegorie?date1=` + date1 + "&date2=" + date2 + "&idCategorie=" + this.svData.idCategorie);
                } else {
        ...
        showFicheStockByDate_Empplacement() {
            var date1 = this.dates[0];
            var date2 = this.dates[1];
            if (date1 <= date2) {

                if (this.svData.idEmplacement != "") {
                    window.open(`${this.apiBaseURL}/pdf_fiche_stock_logistique_emplacement?date1=` + date1 + "&date2=" + date2 + "&idEmplacement=" + this.svData.idEmplacement);
                } else {
                    this.showError("Veillez selectionner le service svp");
                }
        ...
        async GetMedicament() {
            this.isLoading(true);
            await axios
                .get(`${this.apiBaseURL}/fetch_tlog_produit_2`)
                .then((res) => {
        ...
        showFicheStockByDate() {
            ...
            if (date1 <= date2) {
                ...
                window.open(`${this.apiBaseURL}/pdf_fiche_stock_logistique?date1=` + date1 + "&date2=" + date2);
